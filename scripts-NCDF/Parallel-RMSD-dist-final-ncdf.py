...
with open('data.txt', mode='w') as file:
	...
	# Coverting DCD files into NCDF files
        ...
        with mda.coordinates.TRJ.NCDFWriter('newtraj.ncdf', u.trajectory.n_atoms, multiframe=True) as W:
        ...
        for i in block_size:      # changing blocks
            for j in range(1,6):    # changing files (5 files per block size)
                ...
                my_path = os.path.normpath(os.path.join(os.getcwd(), longNCDF1))
                print (my_path)
                longNCDF1 = os.path.abspath(my_path)
                # Define a new universe with the new trajectory
                u = mda.Universe(PSF, longNCDF1)
		print(u)
