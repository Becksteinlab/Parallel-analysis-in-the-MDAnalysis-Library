...
def com_parallel_dask(ag, n_blocks):
    ...
    for iblock in range(n_blocks):
        ...
        out = delayed(block_rmsd, pure=True)(ag, ref0, start=start, stop=stop, step=step)
        blocks.append (out[0])
        t_comp.append (out[1])
...
with open('data.txt', mode='w') as file:
    ...
    for k in traj_size: # we have 3 trajectory sizes
        ...
        # Coverting DCD files into NCDF files
        longNCDF = 'newtraj.ncdf'
        with mda.coordinates.TRJ.NCDFWriter('newtraj.ncdf', u1.trajectory.n_atoms, multiframe=True) as W:
            for ts in u1.trajectory:
                W.write(ts)
        ...
        for i in block_size:      # changing blocks
            for j in range(1,6):    # changing files (5 files per block size)
                longNCDF1 = 'newtraj{}.ncdf'.format(ii)
                copyfile(longNCDF, longNCDF1)
                u = mda.Universe(PDB, longNCDF1)
		print(u)
                ...
                total = delayed (total)
                start = time.time()
                output = total.compute(get=dask.multiprocessing.get)
                tot_time = time.time()-start
                file.write("XTC{} {} {} {} {} {} {} {}\n".format(k, i, j, output [1], output [2], output [3], output [4], tot_time))
		file.flush()
                os.remove('newtraj{}.ncdf'.format(ii))
                ii = ii+1
